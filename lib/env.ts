...
export function env() { const parsed = schema.safeParse(process.env); if (!parsed.success) throw new Error(`Missing environment: ${parsed.error.issues.map((i: { path: Array<string | number> })=>i.path.join(".")).join(", ")}`); return parsed.data; }
